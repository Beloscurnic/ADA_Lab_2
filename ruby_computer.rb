...
def solve_crypto_puzzle(string, difficulty, id_pocess, quantity)
...
begin
  queue.subscribe(block: true) do |_delivery_info, properties, payload|
	...
	id_pocess = json_payload['id_pocess']
    quantity = json_payload['quantity']
    solution = solve_crypto_puzzle(string, difficulty, id_pocess, quantity)
